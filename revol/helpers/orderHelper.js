...
exports.prep_data = function(data) {
    let keys = ["customer", "date", "delivery_point", "code", "status", "notes", "product", "quantity",
                "price", "createdAt", "customer_name",
                 "product_name", "package_date", "packaged_by", "delivery_date", "delivered_by"]
    var returner = {};
...
exports.search = function (param, limit, offset) {
    let final_query= []
    let fields = ["code", "delivery_point", "notes", "customer_name", "product_name"];
      ...
      { $or: [{c: 1}, {d: 1}] }
        ...
        for (var j = 0; j < split_param.length; j++) {
            ...
            for(var k = 0; k < fields.length; k++){
                var f = fields[k];
                var obj = {}
                //turns out $options cannot be used with fields of type Number
