...
router.post('/save', function (req, res) {
    ...
    if (id == undefined || id == null) {
        ...
        ad.save()
            .then(result => {
                ActivitiesHelper.save(req.body.display_name + " placed order for " + req.body.customer_name);
        ...
        Model.findByIdAndUpdate({ _id: req.body._id }, obj)
            .then(result => {
                res.json({ status: '1', data: result });
            })
            .catch(err => {
...
router.post('/saveBulk', function (req, res) {
    ...
    //if editing param was not supplied, then we're not editing
    var editing = req.body.editing != undefined
    ...
    for (var i = 0; i < product_array.length; i++) {
        var obj = {
            ...
            code: req.body.code,//may be supplied if editing
            added_by: req.body.added_by,
            date: req.body.date,//may be supplied if editing
            customer: customer,
            status: status,
    ...
    //if editing, delete existing first
    if (editing) {
        Model.deleteMany({ code: req.body.code })
            .then(result => {
            })
            .catch(err => {
                console.log(err);
...
router.post('/updateBulkStatus', function (req, res) {
    ...
    async.each(objects, function (obj, next) {
        ...
        ActivitiesHelper.save(req.body.display_name + " " + status + " packaged order for " + req.body.customer_name);
...
router.post('/delete', function (req, res) {
    ...
    Model.remove({ "_id": id })
        .then(result => {
            ActivitiesHelper.save(req.body.display_name + " deleted order  " + req.body.code);
...
router.post('/deleteByCode', function (req, res) {

    var codes = req.body.codes;//comma-separated-string
    var code_array = codes.split(",");

    var async = require('async');
    async.each(code_array, function (obj, next) {

        Model.deleteMany({ code: obj })
            .then(result => {
                next();
            })
            .catch(err => {
        ...
        ActivitiesHelper.save(req.body.display_name + "  deleted orders: " + code_array.toString());
...
router.get('/count', function (req, res) {
    ...
    Model.count(obj)
        .then(result => {
            res.json({ status: "1", num_orders: result });
            //console.log(result)
        })
        .catch(err => {
            res.json({ status: "-1" });
...
//get the count of each status grouped by order code
router.get('/getStatusCounts', function (req, res) {
    // var obj = {}
    // if (req.query.status != undefined) {

    //     obj.status = req.query.status
    // }
    //attempt to get the number of unique codes where status is what was specified
    var obj = Helper.prep_data(req.query)
    var status = req.query.status
    Model.find(obj)
        .then(result => {
            //get the distinct codes
            var items = [];
            result.map(r => {
                if (items.indexOf(r.code) == -1) {
                    items.push(r.code)
                }
            })
            res.json({ status: "1", num_orders: items.length });
            //console.log(result)
        })
        .catch(err => {
            res.json({ status: "-1" });
